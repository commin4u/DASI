package com.uni.order.api.controller;


import com.uni.order.api.dto.request.OrderRequestDto;
import com.uni.order.service.CalculatorService;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;


@RestController
@RequestMapping(value = "/api/price")
@RequiredArgsConstructor
public class PriceController {

    private final CalculatorService calculatorService;

    @PostMapping("/{videoGameId}")
    public Double getOrderPrice(@RequestBody @Valid OrderRequestDto orderRequestDto, @PathVariable Long videoGameId) {
        return calculatorService.calculatePrice(orderRequestDto, videoGameId);
    }

}
